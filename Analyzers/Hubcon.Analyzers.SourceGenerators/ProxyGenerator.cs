    ...
    private JsonTypeInfo Create_System_Text_Json_JsonElement(JsonSerializerOptions options)
    {
        ...
        private static string GenerateSubscriptionHandlerFactory(IEnumerable<INamedTypeSymbol> interfaces, IList<ITypeSymbol> typesToSerialize)
        {
            ...
            var controllerContracts = interfaces.Where(i =>
                ...
                (i.ToDisplayString() == "Hubcon.Shared.Abstractions.Standard.Interfaces.IControllerContract" ||
                 i.AllInterfaces.Any(ai => ai.ToDisplayString() == "Hubcon.Shared.Abstractions.Standard.Interfaces.IControllerContract")));
            ...
            var genericTypes = controllerContracts
                ...
                // Filtro más robusto para la interfaz
                .Where(type => type.OriginalDefinition.ToDisplayString() == "Hubcon.Shared.Abstractions.Interfaces.ISubscription<T>")
                .Select(type => type.TypeArguments.First()); // AQUÍ obtenemos el T (ej: int?)
            ...
            sb.AppendLine("{");
            sb.AppendLine("    public static object Create(Type type, object config) => type switch");
            sb.AppendLine("    {");

            // 3. Deduplicación y Registro en el HashSet
            ...
            {
                // Al agregar 'type' aquí, estamos agregando el T (ej: int?), no el ISubscription
                typesToSerialize.Add(type);

                string fullName = type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);

                // Generación del branch del switch
